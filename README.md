...
- A few silent fallbacks affect interpretation:
  - `fe3d` buckling: if no positive eigenvalue is found (or the eigenvalue is < 5% of σ₀), the buckling result is discarded and FPF — or, in pure-buckling failure, σ₀ — is reported instead. The reason is now surfaced as a string in `AnalysisResults.notes` (also rendered in the Streamlit Summary tab and in `result.summary()` / `result.to_dict()`), so a `knockdown` of 1.0 from `fe3d` is distinguishable from "buckling solve unconverged" by inspecting `result.notes`.
